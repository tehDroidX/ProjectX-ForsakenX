...
set INCS=/I"%SMOKE%\compat" /I"%SMOKE%\compat\SDL" /I"%ENG%" /I"%PV%\deps\include"
cd /d "%ENG%"
echo === compiling 103 sources ===
...
echo === compiling 2 patched sources ===
cl /c /nologo /MD /std:c11 /W1 %DEFS% %INCS% /Fo"%OBJ%\\" "%PV%\patched\stats.c" "%PV%\patched\xmem.c" >"%PV%\cc-patched.log" 2>&1
