...
bool LoadStartPoints( void )
{
	...
	if( fp != NULL )
	{
		...
		/* STUDIO_PREVIEW: -startpos:x,y,z overrides every spawn so the
		   player boots at the requested location regardless of group. */
		{
			extern bool  cli_startpos_set;
			extern float cli_startpos_x, cli_startpos_y, cli_startpos_z;
			if( cli_startpos_set && num_start_positions > 0 )
			{
				int _sp;
				for( _sp = 0; _sp < num_start_positions; _sp++ )
				{
					StartPositions[ _sp ].Pos.x = cli_startpos_x;
					StartPositions[ _sp ].Pos.y = cli_startpos_y;
					StartPositions[ _sp ].Pos.z = cli_startpos_z;
				}
				DebugPrintf("cli: STUDIO_PREVIEW overrode %d start positions to (%f, %f, %f)\n",
					(int)num_start_positions, cli_startpos_x, cli_startpos_y, cli_startpos_z);
			}
		}
