...
/* ---- STUDIO_PREVIEW quick-start additions ---- */
/* -level:<name>   : direct-boot the level in single-player (see title.c StudioPreview_BootSinglePlayer) */
/* -startpos:x,y,z : override player spawn position after level load (see restart.c) */
char  cli_level_name[64]   = "";
bool  cli_level_set        = false;
bool  cli_direct_boot      = false;   /* title.c: skip host menu, load level in single-player now */
bool  cli_startpos_set     = false;
float cli_startpos_x       = 0.0f;
float cli_startpos_y       = 0.0f;
float cli_startpos_z       = 0.0f;

#ifdef DEDICATED_SERVER
/* -dedicated:<config.cfg> : boot straight into a hosted match (see title.c Dedicated_BootHost) */
bool cli_dedicated          = false;
char cli_dedicated_cfg[256] = "";
#endif
extern void config_set_str(const char *opt, char* str); /* lua_config.c */
...
static bool ParseCommandLine(char* lpCmdLine)
{
	...
	/* STUDIO_PREVIEW: pull -level:<name> out of the raw command line BEFORE
	   the strtok() pass below.  strtok's delimiter set (" -+'\"") treats '-'
	   as the start of the next switch, so hyphenated level names were cut at
	   the first '-' (-level:fzk-cubes -> "fzk") and the remainder ("cubes")
	   was re-parsed as a switch.  Grab the full name (up to whitespace/quote)
	   here, then blank the region so the token loop never sees it.          */
	{
		char * lvl = strstr( cmdline, "-level:" );
		if ( lvl )
		{
			char * name = lvl + 7;                    /* strlen("-level:")   */
			size_t len  = strcspn( name, " \t'\"" );  /* full name length    */
			size_t copy = len;

			if ( copy > sizeof(cli_level_name) - 1 )
				copy = sizeof(cli_level_name) - 1;
			strncpy( cli_level_name, name, copy );
			cli_level_name[copy] = 0;

			/* hide the whole switch from the strtok loop below */
			memset( lvl, ' ', 7 + len );

			if ( copy )
			{
				cli_level_set   = true;
				cli_direct_boot = true;   /* title.c: bypass host menu, load level now */
				/* config was already loaded by GetGamePrefs() before ParseCommandLine();
				   keep LevelName in sync so any menu fallback still preselects it.     */
				config_set_str( "LevelName", cli_level_name );
				DebugPrintf("cli: STUDIO_PREVIEW level '%s' selected; single-player direct-boot\n", cli_level_name);
			}
			else
			{
				DebugPrintf("cli: STUDIO_PREVIEW -level: given with empty name; ignored\n");
			}
		}
	}
...
#ifdef DEDICATED_SERVER
	/* -dedicated:<path> : pull the config path out of the raw command line
	   before the strtok() pass mangles it. Grab to end-of-line so paths with
	   spaces work - the supervisor always passes -dedicated: as the LAST arg. */
	{
		char * ded = strstr( cmdline, "-dedicated:" );
		if ( ded )
		{
			char * path = ded + 11;                   /* strlen("-dedicated:") */
			size_t len  = strcspn( path, "\r\n" );    /* rest of the line      */
			size_t copy = len;
			while ( copy > 0 && ( path[copy-1] == ' ' || path[copy-1] == '\t' ) )
				copy--;                                /* trim trailing space   */
			if ( copy > sizeof(cli_dedicated_cfg) - 1 )
				copy = sizeof(cli_dedicated_cfg) - 1;
			strncpy( cli_dedicated_cfg, path, copy );
			cli_dedicated_cfg[copy] = 0;
			memset( ded, ' ', 11 + len );             /* hide from strtok loop */
			if ( cli_dedicated_cfg[0] )
			{
				cli_dedicated = true;
				render_info.fullscreen = false;       /* windowed dedicated host */
				DebugPrintf("cli: DEDICATED_SERVER config '%s'; hosted-match direct-boot\n", cli_dedicated_cfg);
			}
		}
	}
    ...
    while(option != NULL )
	{
		...
		else 
		{
			...
			// STUDIO_PREVIEW: -level:<name> is handled in a pre-pass above (before
			// strtok), because the tokenizer's '-' delimiter truncated hyphenated
			// level names. No token branch for it here anymore.

			// STUDIO_PREVIEW: -startpos:x,y,z override player spawn (see restart.c LoadStartPoints)
			else if ( sscanf( option, "startpos:%f,%f,%f", &cli_startpos_x, &cli_startpos_y, &cli_startpos_z ) == 3 )
			{
				cli_startpos_set = true;
				DebugPrintf("cli: STUDIO_PREVIEW startpos override (%f, %f, %f)\n",
					cli_startpos_x, cli_startpos_y, cli_startpos_z);
			}
