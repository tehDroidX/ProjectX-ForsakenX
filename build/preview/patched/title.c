...
#ifdef STUDIO_PREVIEW
/* ---- STUDIO_PREVIEW direct single-player boot ---------------------
   Launched with -level:<name> we want the engine to drop STRAIGHT into that
   level in single-player (a solo 6DOF fly-through preview) instead of stopping
   at the multiplayer host menu.  This helper is invoked once from
   DisplayTitle()'s first-time init, in place of the normal QuickStart menu
   setup.  It resolves the requested level name to a level index, points
   NewLevelNum at it, and kicks off the stock single-player start path
   (StartASinglePlayerGame -> STATUS_StartingSinglePlayer -> ChangeLevel), which
   the oct2.c state machine carries through to STATUS_SinglePlayer (in-level).  */
extern bool  cli_direct_boot;               /* main.c  */
extern char  cli_level_name[64];            /* main.c  */
extern int   FindSameLevel( char * Name );  /* networking.c */
extern bool  GodMode;                       /* controls.c   */

void StudioPreview_BootSinglePlayer( void )
{
	int idx = -1;

	/* Most preview levels are registered in the multiplayer list (battle.dat). */
	if( InitLevels( MULTIPLAYER_LEVELS ) )
		idx = FindSameLevel( cli_level_name );

	/* Fall back to the single-player campaign list (mission.dat). */
	if( idx < 0 && InitLevels( SINGLEPLAYER_LEVELS ) )
		idx = FindSameLevel( cli_level_name );

	if( idx < 0 )
	{
		/* No silent fallback to level index 0: a preview of the wrong level is
		   worse than a clear failure.  Log the error and quit (exit(1) is the
		   established fatal-error path in this file).                          */
		DebugPrintf( "STUDIO_PREVIEW: ERROR: level '%s' not found in battle.dat (MULTIPLAYER_LEVELS) or mission.dat (SINGLEPLAYER_LEVELS); quitting.\n", cli_level_name );
		Msg( "STUDIO_PREVIEW: level '%s' not found in battle.dat or mission.dat\n", cli_level_name );
		exit( 1 );
	}

	DebugPrintf( "STUDIO_PREVIEW: direct-boot single-player into level '%s' (index %d)\n", cli_level_name, idx );

	NewLevelNum             = (int16_t) idx;
	LevelList.selected_item = idx;

	/* Invulnerability so enemy fire in campaign levels can't end the preview.
	   Set the flag directly (not ToggleGodMode) to avoid GivemeAllWeapons()
	   touching a ship that has not been spawned yet.                          */
	GodMode = true;

	/* Sets up the solo game and MyGameStatus = STATUS_StartingSinglePlayer. */
	StartASinglePlayerGame( NULL );

	DebugPrintf( "STUDIO_PREVIEW: single-player start kicked off, NewLevelNum=%d\n", (int) NewLevelNum );
}
#endif /* STUDIO_PREVIEW */

#ifdef DEDICATED_SERVER
/* ---- DEDICATED_SERVER autostart (forsaken-dedicated-server/client-patch) ----
   Launched with  -dedicated:<config.cfg>  the engine boots STRAIGHT into a
   hosted multiplayer match (no menu) that stays up and clients can join.
   Mirrors StudioPreview_BootSinglePlayer above, but drives the HOST path.
   It sets the same globals the host menus set, then calls the REAL, tested
   StartAHostSession() (multiplayer.c:270) with a crafted menu item (so the
   internal MenuChange() doesn't deref NULL), and GoToSynchup() to kick the
   level load - exactly the two steps a human host clicks.                     */
extern bool  cli_dedicated;                 /* main.c */
extern char  cli_dedicated_cfg[256];        /* main.c */
extern int   FindSameLevel( char * Name );  /* networking.c */
extern void  StartAHostSession( MENUITEM * Item );  /* multiplayer.c */
extern void  GoToSynchup( MENUITEM * Item );        /* multiplayer.c */
extern MENU  MENU_NEW_HostWaitingToStart;           /* title.c menu table   */

typedef struct {
    char level[64], mode[24], name[16];
    int  localPort, maxKills, timeLimitMin, randomPickups, resetKillsPerLevel, numTeams, spectator;
} dedicated_cfg_t;

/* minimal key/value config reader: one "key value" pair per line, '#' starts a comment. */
static void dedicated_load_config( const char * path, dedicated_cfg_t * c )
{
    FILE * fp; char line[512];
    strcpy(c->level,"pyramid"); strcpy(c->mode,"deathmatch"); strcpy(c->name,"DediServer");
    c->localPort=2300; c->maxKills=20; c->timeLimitMin=10;
    c->randomPickups=1; c->resetKillsPerLevel=1; c->numTeams=2; c->spectator=0;
    fp = fopen( path, "r" );
    if( !fp ){ DebugPrintf("DEDICATED: cannot open config '%s', using defaults\n", path); return; }
    while( fgets( line, sizeof(line), fp ) )
    {
        char key[64], val[256];
        if( line[0]=='#' || line[0]=='\n' ) continue;
        if( sscanf( line, "%63s %255[^\r\n]", key, val ) != 2 ) continue;
        if      (!strcasecmp(key,"level"))              strncpy(c->level, val, sizeof(c->level)-1);
        else if (!strcasecmp(key,"mode"))               strncpy(c->mode,  val, sizeof(c->mode)-1);
        else if (!strcasecmp(key,"name"))               strncpy(c->name,  val, sizeof(c->name)-1);
        else if (!strcasecmp(key,"localPort"))          c->localPort          = atoi(val);
        else if (!strcasecmp(key,"maxKills"))           c->maxKills           = atoi(val);
        else if (!strcasecmp(key,"timeLimitMin"))       c->timeLimitMin       = atoi(val);
        else if (!strcasecmp(key,"randomPickups"))      c->randomPickups      = atoi(val);
        else if (!strcasecmp(key,"resetKillsPerLevel")) c->resetKillsPerLevel = atoi(val);
        else if (!strcasecmp(key,"spectator"))          c->spectator          = atoi(val);
        else if (!strcasecmp(key,"numTeams"))           c->numTeams           = atoi(val);
    }
    fclose( fp );
    DebugPrintf("DEDICATED: config level=%s mode=%s port=%d frag=%d time=%d\n",
                c->level, c->mode, c->localPort, c->maxKills, c->timeLimitMin);
}

/* GUI mode string -> engine GameType enum (title.h order) */
static int dedicated_game_type( const char * mode )
{
    if(!strcasecmp(mode,"team"))             return GAME_Team;
    if(!strcasecmp(mode,"capture-the-flag")) return GAME_CTF;
    if(!strcasecmp(mode,"bounty"))           return GAME_BountyHunt;
    return GAME_Normal; /* deathmatch / free-for-all */
}

void Dedicated_BootHost( void )
{
    dedicated_cfg_t cfg;
    MENUITEM item;
    int idx;

    dedicated_load_config( cli_dedicated_cfg, &cfg );

    /* resolve the level name to an index (mirror StudioPreview) */
    idx = -1;
    if( InitLevels( MULTIPLAYER_LEVELS ) )
        idx = FindSameLevel( cfg.level );
    if( idx < 0 )
    {
        DebugPrintf("DEDICATED: ERROR level '%s' not in battle.dat; quitting.\n", cfg.level);
        Msg("DEDICATED: level '%s' not found in battle.dat\n", cfg.level);
        exit( 1 );
    }
    LevelList.selected_item = idx;

    /* push config into the same globals the host menus set */
    strncpy( biker_name, cfg.name, sizeof(biker_name)-1 );
    sprintf( local_port_str.text, "%d", cfg.localPort );
    GameType = dedicated_game_type( cfg.mode );
    CLAMP( GameType, MAX_GAMETYPE );
    MaxKillsSlider.value = (float) cfg.maxKills;
    MyTimeLimit.value    = (float) cfg.timeLimitMin;
    MyRandomPickups      = cfg.randomPickups     ? true : false;
    MyResetKillsPerLevel = cfg.resetKillsPerLevel ? true : false;

    /* step 1: the exact "Start game" path (sets IsHost, network_setup/host,
       SetupNetworkGame, STATUS_StartingMultiplayer, tracker beacon, ...).
       A crafted item keeps the internal MenuChange() from deref'ing NULL.   */
    memset( &item, 0, sizeof(item) );
    item.Value = (void*) &MENU_NEW_HostWaitingToStart;
    StartAHostSession( &item );

    /* step 2: the "everyone ready, go" action -> clears the menu so the
       oct2.c state machine loads the level. (Its Item arg is unused here.)  */
    GoToSynchup( NULL );

    /* optional: host as spectator (WATCH_MODE) instead of an active player.
       WatchPlayerSelect.value != WhoIAm makes the host ship enter WATCH_MODE
       every frame (ships.c:921). EXPERIMENTAL - if it doesn't stick past ship
       spawn, re-assert this value after the level finishes loading.          */
    if ( cfg.spectator )
    {
        extern SLIDER WatchPlayerSelect;                 /* ships.c / title.c */
        WatchPlayerSelect.value = (float)( MAX_PLAYERS + 1 );  /* free-cam watch */
        DebugPrintf("DEDICATED: spectator mode - host is not an active player\n");
    }

    DebugPrintf("DEDICATED: hosting '%s' (type %d) port %d, level idx %d - waiting for joiners\n",
                cfg.level, GameType, cfg.localPort, idx);
}
...
#ifdef DEDICATED_SERVER
		/* -dedicated:<cfg> requests a direct boot into a HOSTED match; skip the
		   whole QuickStart menu machine and start hosting now.                 */
		if ( cli_dedicated )
		{
			Dedicated_BootHost();
		}
...
#endif
#ifdef STUDIO_PREVIEW
		/* STUDIO_PREVIEW: -level:<name> requests a direct single-player boot;
		   skip the whole QuickStart menu machine and load the level now.      */
		if ( cli_direct_boot )
		{
			StudioPreview_BootSinglePlayer();
		}
		else
#endif
		switch ( QuickStart )
